...
try{
  ...
  try{
    ...
    await page.locator('.toolbarMore>summary').click();await page.locator('[data-path-input]').fill(path.join(process.cwd(),'fixture'));await page.locator('[data-action="open-project"]').click();await frameLocator.waitFor({state:'attached'});let frame=await refreshFrame();await closeToolbarOverlays();
