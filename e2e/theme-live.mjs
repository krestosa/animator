...
try{
  ...
  try{
    ...
    frame=await openRemote('https://www.google.com/');await frame.locator('body').waitFor({timeout:20000});assert.equal(await theme.inputValue(),'light','session theme did not carry into Google');await waitUntil(async()=>await frame.evaluate(()=>matchMedia('(prefers-color-scheme: light)').matches&&!matchMedia('(prefers-color-scheme: dark)').matches),'Google preview did not inherit Light override');
