...
try{
  ...
  try{
    ...
    const dragX=railBox.x+Math.min(180,railBox.width*.45),dragY=railBox.y+railBox.height/2;await page.mouse.move(dragX,dragY);await page.mouse.down();await page.waitForTimeout(80);
    ...
    await recordButton.click();await waitUntil(async()=>/STOPPED/.test(await recordBadge.textContent()??''),'record button did not stop capture');const stoppedClips=await page.locator('.v2Clip').count();await frame.evaluate(()=>{const target=document.querySelector('#box');if(target)target.animate([{outlineOffset:'0px'},{outlineOffset:'12px'}],{duration:80});});await page.waitForTimeout(260);assert.equal(await page.locator('.v2Clip').count(),stoppedClips,'animation was captured after recording stopped');
    await recordButton.click();await waitUntil(async()=>/REC/.test(await recordBadge.textContent()??''),'record button did not restart capture');await waitUntil(async()=>/Live/i.test(await page.locator('[data-preview-frame-label]').textContent()??''),'restarting record did not release timeline back to Live');await page.waitForTimeout(350);const resumedClips=await page.locator('.v2Clip').count();await frame.evaluate(()=>{const target=document.querySelector('#box');if(target)target.animate([{borderRadius:'0px'},{borderRadius:'17px'}],{duration:600,easing:'linear'});});await waitUntil(async()=>await page.locator('.v2Clip').count()>resumedClips,'recording resumed but new motion was not captured');
