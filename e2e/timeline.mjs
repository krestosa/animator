...
try{
  ...
  try{
    ...
    const switchTheme=async expected=>{await themeSelect.selectOption(expected);await waitUntil(async()=>await themeSelect.inputValue()===expected,`theme control did not switch to ${expected}`);if(expected==='auto')await waitUntil(async()=>!new URL(await frameLocator.getAttribute('src')).searchParams.has('__animator_color_scheme'),'Auto left an override query on preview URL');else await waitUntil(async()=>new URL(await frameLocator.getAttribute('src')).searchParams.get('__animator_color_scheme')===expected,`preview URL did not apply ${expected}`);await page.waitForTimeout(180);frameHandle=await frameLocator.elementHandle();frame=await frameHandle?.contentFrame();assert(frame,`preview frame missing after ${expected} theme reload`);await frame.locator('body').waitFor();};
    ...
    await frame.evaluate(()=>location.reload());await page.waitForTimeout(220);frameHandle=await frameLocator.elementHandle();frame=await frameHandle?.contentFrame();assert(frame,'preview frame missing after reload');await frame.locator('body').waitFor();assert(await frame.evaluate(()=>matchMedia('(prefers-color-scheme: dark)').matches),'same-page reload lost the active theme override');
