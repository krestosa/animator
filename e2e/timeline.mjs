...
try{
  ...
  try{
    ...
    await group.locator('[data-v2-toggle]').click();await waitUntil(async()=>await page.locator('.v2InstanceRow').count()===0,'repeat-pop group did not collapse before isolation');
    const motionIsolation=page.locator('[data-isolate="animation"]');await motionIsolation.click();await waitUntil(async()=>await page.locator('.v2GroupRow:not([hidden])').count()>=1,'motion isolation hid every collapsed group row');
    assert((await page.locator('.v2GroupRow:not([hidden])').count())>=1,'motion isolation hid the selected group row');await page.locator('[data-isolate="all"]').click();
    ...
    const focusVisual=await frame.locator('[data-animator-focus-overlay]').evaluate(node=>({border:getComputedStyle(node).borderTopWidth,shadow:getComputedStyle(node).boxShadow}));assert.equal(focusVisual.border,'0px','Focus still renders a visible border');assert(/9999px/.test(focusVisual.shadow),'Focus no longer darkens only the surrounding area');
    ...
    const camera=await frameLocator.evaluate(node=>({transform:node.style.transform,scale:Number(node.dataset.cameraScale),pointer:getComputedStyle(node).pointerEvents}));assert(camera.scale>=1,`camera unexpectedly zoomed out: ${JSON.stringify(camera)}`);assert(/translate3d\(.+scale\(/.test(camera.transform),'camera zoom is not a translated viewport transform');assert.equal(camer.pointer,'auto','camera zoom blocked normal UI');
    const zoomedScroll=await page.locator('.stage').evaluate(node=>{node.scrollTop+=120;return node.scrollTop;});assert(zoomedScroll>0,'camera zoom blocked normal scroll');await page.locator('[data-magnify-mode]').click();await waitUntil(async()=>await frameLocator.getAttribute('data-camera-zoom')==='off','camera zoom did not exit');
    ...
    await recordButton.click();await waitUntil(async()=>await frame.evaluate(()=>window.__ANIMATOR_CAPTURE_GATE__?.recording===true),'preview capture gate did not resume after REC');const resumedClips=await page.locator('.v2Clip').count();await frame.evaluate(()=>{const target=document.querySelector('#box');if(target)target.animate([{borderRadius:'0px'},{borderRadius:'17px'}],{duration:600,easing:'linear'});});await waitUntil(async()=>await page.locator('.v2Clip').count()>resumedClips,'recording resumed but new motion was not captured');await waitUntil(async()=>/REC/.test(await recordBadge.textContent()??''),'record button did not show REC');await waitUntil(async()=>/Live/i.test(await page.locator('[data-preview-frame-label]').textContent()??''),'restarting record did not release timeline back to Live');
    ...
    const themeSrc=await frameLocator.getAttribute('src'),themeSentinel='timeline-'+Math.random().toString(36).slice(2);await frame.evaluate(value=>window.__themeSentinel=value,themeSentinel);
    const switchTheme=async expected=>{await themeSelect.selectOption(expected);await waitUntil(async()=>await themeSelect.inputValue()===expected,`theme control did not switch to ${expected}`);await page.waitForTimeout(180);assert.equal(await frameLocator.getAttribute('src'),themeSrc,`theme ${expected} changed preview URL/reloaded the document`);assert.equal(await frame.evaluate(()=>window.__themeSentinel),themeSentinel,`theme ${expected} replaced the preview document`);};
    ...
    await frame.evaluate(()=>location.reload());await page.waitForTimeout(220);frameHandle=await frameLocator.elementHandle();frame=await frameHandle?.contentFrame();assert(frame,'preview frame missing after reload');await frame.locator('body').waitFor();await waitUntil(async()=>await frame.evaluate(()=>matchMedia('(prefers-color-scheme: dark)').matches),'same-page reload lost the active session theme');
    assert.equal(await page.evaluate(()=>Object.keys(localStorage).some(key=>key.startsWith('animator.previewColorScheme'))),false,'theme override leaked into persistent localStorage');
    ...
    await waitUntil(async()=>{const src=await frameLocator.getAttribute('src');return !!src&&src!==localSrc&&/^http:\/\/127\.0\.0\.1:\d+\//.test(src);},'remote page did not move to a local instrumented preview');await waitUntil(async()=>await themeSelect.inputValue()==='dark','session theme did not carry into a different parent site');frameHandle=await frameLocator.elementHandle();frame=await frameHandle?.contentFrame();assert(frame,'remote frame missing');await frame.locator('#remote-card').waitFor();await waitUntil(async()=>await frame.evaluate(()=>matchMedia('(prefers-color-scheme: dark)').matches),'remote preview did not receive active session theme');assert(await frame.evaluate(()=>window.__remoteLoaded===true&&document.documentElement.dataset.remoteScript==='ok'),'remote relative JS did not load');assert.equal(await frame.locator('#remote-card').evaluate(node=>getComputedStyle(node).animationName),'remote-in','remote relative CSS did not load');

    await page.locator('.toolbarMore>summary').click();await page.locator('[data-path-input]').fill(path.join(process.cwd(),'fixture'));await page.locator('[data-action="open-project"]').click();await waitUntil(async()=>await themeSelect.inputValue()==='dark','returning to the original project lost the session theme');
