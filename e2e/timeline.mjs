...
try{
  ...
  try{
    ...
    const themeButton=page.locator('[data-preview-theme]');await themeButton.waitFor({state:'visible'});assert.equal(await themeButton.getAttribute('data-mode'),'system','preview theme did not default to System');
    const switchTheme=async expected=>{await themeButton.click();await waitUntil(async()=>await themeButton.getAttribute('data-mode')===expected,`theme button did not switch to ${expected}`);await waitUntil(async()=>new URL(await frameLocator.getAttribute('src')).searchParams.get('__animator_color_scheme')===expected,`preview URL did not apply ${expected}`);await page.waitForTimeout(180);frameHandle=await frameLocator.elementHandle();frame=await frameHandle?.contentFrame();assert(frame,`preview frame missing after ${expected} theme reload`);await frame.locator('body').waitFor();};
    await switchTheme('light');assert(await frame.evaluate(()=>matchMedia('(prefers-color-scheme: light)').matches&&!matchMedia('(prefers-color-scheme: dark)').matches),'site JS did not detect forced light mode');assert.equal(await frame.locator('body').evaluate(node=>getComputedStyle(node,'::after').content),'"light"','site CSS did not detect forced light mode');
    await switchTheme('dark');assert(await frame.evaluate(()=>matchMedia('(prefers-color-scheme: dark)').matches&&!matchMedia('(prefers-color-scheme: light)').matches),'site JS did not detect forced dark mode');assert.equal(await frame.locator('body').evaluate(node=>getComputedStyle(node,'::before').content),'"dark"','site CSS did not detect forced dark mode');assert.equal(await page.evaluate(()=>localStorage.getItem('animator.previewColorScheme')),'dark','preview theme preference was not persisted');
    await switchTheme('system');assert.equal(await page.evaluate(()=>localStorage.getItem('animator.previewColorScheme')),'system','system preference was not persisted');
