...
try{
  ...
  try{
    ...
    const camera=await frameLocator.evaluate(node=>({transform:node.style.transform,scale:Number(node.dataset.cameraScale),pointer:getComputedStyle(node).pointerEvents}));assert(camera.scale>=1,`camera unexpectedly zoomed out: ${JSON.stringify(camera)}`);assert(/translate3d\(.+scale\(/.test(camera.transform),'camera zoom is not a translated viewport transform');
