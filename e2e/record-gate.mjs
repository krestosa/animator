...
const remote=createServer((req,res)=>{
  if(req.url==='/'){documentHits++;res.writeHead(200,{'content-type':'text/html'});res.end(`<!doctype html><html><head><style>html,body{margin:0;width:2600px;min-height:3200px}#marker{position:absolute;left:1800px;top:2400px;width:200px;height:120px;background:#468}</style></head><body><div id="marker"></div><script>window.__wheel=0;window.__scroll=0;addEventListener('wheel',()=>window.__wheel++);addEventListener('scroll',()=>window.__scroll++);</script></body></html>`);return;}
...
try{
  ...
  try{
    ...
    const pageInput=await frame.evaluate(()=>({wheel:window.__wheel,scroll:window.__scroll,x:scrollX,y:scrollY}));assert.equal(pageInput.wheel,0,'locked preview delivered wheel to the web');assert.equal(pageInput.scroll,0,'locked preview delivered scroll events to the web');assert(pageInput.x>0&&pageInput.y>0,'locked preview did not pan both axes');
