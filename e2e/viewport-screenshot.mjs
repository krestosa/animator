...
async function readPngRatio(page,base64,xRatio,yRatio){return page.evaluate(async({base64,xRatio,yRatio})=>{const blob=await (await fetch(`data:image/png;base64,${base64}`)).blob(),bitmap=await createImageBitmap(blob),canvas=document.createElement('canvas');canvas.width=bitmap.width;canvas.height=bitmap.height;const context=canvas.getContext('2d');context.drawImage(bitmap,0,0);return Array.from(context.getImageData(Math.min(bitmap.width-1,Math.floor(bitmap.width*xRatio)),Math.min(bitmap.height-1,Math.floor(bitmap.height*yRatio)),1,1).data);},{base64,xRatio,yRatio});}
function assertExactViewport(clipboard,{marker,background}){assert(clipboard.types.includes('image/png'),`Clipboard did not contain image/png: ${clipboard.types.join(', ')}`);assert.equal(clipboard.width,1100,'Screenshot width did not match visible viewport');assert.equal(clipboard.height,700,'Screenshot height did not match visible viewport');if(marker==='magenta')assert(clipboard.marker[0]>240&&clipboard.marker[1]<20&&clipboard.marker[2]>150,`Screenshot did not contain current-frame marker: ${clipboard.marker.join(',')}`);else assert(clipboard.marker[0]<20&&clipboard.marker[1]>210&&clipboard.marker[2]>235,`Proxy screenshot did not contain current-frame marker: ${clipboard.marker.join(',')}`);if(background)assertPixelClose(clipboard.background,background,12,'Proxy screenshot background differed from the visible viewport');else assert(clipboard.background[0]<40&&clipboard.background[1]<50&&clipboard.background[2]<60,`Screenshot lost Auto dark theme: ${clipboard.background.join(',')}`);}
