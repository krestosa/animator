...
try{
  ...
  try{
    ...
    const proxyRendered=await proxyFrame.evaluate(()=>({marker:getComputedStyle(document.querySelector('#exact-frame-marker')).backgroundColor,background:getComputedStyle(document.body).backgroundColor}));assert.equal(proxyRendered.marker,'rgb(0, 229, 255)','Proxy visible-frame marker was not applied');
    const visibleBackground=parseCssColor(proxyRendered.background);assert.deepEqual(visibleBackground,[16,24,32,255],'Proxy Auto theme did not render the expected dark background');
...
async function assertSuccessToast(page){const toast=page.locator('[data-viewport-capture-toast="success"]');await toast.waitFor({state:'visible',timeout:35000});assert.equal(await toast.textContent(),'Screenshot copied','Success toast copy mismatch');}
async function readClipboardPixels(page){return page.evaluate(async()=>{const items=await navigator.clipboard.read(),blob=await items[0].getType('image/png'),bitmap=await createImageBitmap(blob),canvas=document.createElement('canvas');canvas.width=bitmap.width;canvas.height=bitmap.height;const context=canvas.getContext('2d');context.drawImage(bitmap,0,0);const marker=Array.from(context.getImageData(30,30,1,1).data),background=Array.from(context.getImageData(Math.min(bitmap.width-1,Math.floor(bitmap.width*.85)),Math.min(bitmap.height-1,Math.floor(bitmap.height*.85)),1,1).data);return{types:items.flatMap(item=>item.types),width:bitmap.width,height:bitmap.height,marker,background};});}
function parseCssColor(value){const numbers=value.match(/[\d.]+/g)?.map(Number)??[];return[Math.round(numbers[0]??0),Math.round(numbers[1]??0),Math.round(numbers[2]??0),numbers.length>3?Math.round((numbers[3]??1)*255):255];}
function assertExactViewport(clipboard,{marker,background}){assert(clipboard.types.includes('image/png'),`Clipboard did not contain image/png: ${clipboard.types.join(', ')}`);assert.equal(clipboard.width,1100,'Screenshot width did not match visible viewport');assert.equal(clipboard.height,700,'Screenshot height did not match visible viewport');if(marker==='magenta')assert(clipboard.marker[0]>240&&clipboard.marker[1]<20&&clipboard.marker[2]>150,`Screenshot did not contain current-frame marker: ${clipboard.marker.join(',')}`);else assert(clipboard.marker[0]<20&&clipboard.marker[1]>210&&clipboard.marker[2]>235,`Proxy screenshot did not contain current-frame marker: ${clipboard.marker.join(',')}`);if(background)assertPixelClose(clipboard.background,background,12,'Proxy screenshot background differed from the rendered document');else assert(clipboard.background[0]<40&&clipboard.background[1]<50&&clipboard.background[2]<60,`Screenshot lost Auto dark theme: ${clipboard.background.join(',')}`);}
function assertPixelClose(actual,expected,tolerance,message){for(let i=0;i<3;i++)assert(Math.abs(actual[i]-expected[i])<=tolerance,`${message}: actual=${actual.join(',')} expected=${expected.join(',')}`);}
