...
import type { BrowserCommand, BrowserEngine, BrowserEvent, BrowserProfile, BrowserRuntimeStatus, BrowserSession, BrowserSessionState, BrowserSnapshotStatus } from './browser/types.js';

export type { BrowserEngine, BrowserProfile, BrowserRuntimeStatus, BrowserSessionState, BrowserSnapshotStatus } from './browser/types.js';
...
export async function openBrowserSession(input:string,options:{width?:number;height?:number;engine?:unknown;profile?:unknown}={}):Promise<{id:string;url:string;title:string;engine:BrowserEngine;profile:BrowserProfile;width:number;height:number;external:boolean}>{
  ...
  const session:BrowserSession={id,browser,context,page,events:[],width,height,destroyed:false,browserClosed:false,headless,recording:true,snapshotStatus:'idle',snapshotVersion:0,snapshotGeneration:0,checkpointBusy:false,sticky:new Map(),navigationVersion:0,engine,profile,url:url.href,title:''};
