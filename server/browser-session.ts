...
export async function openBrowserSession(input:string,options:{width?:number;height?:number;engine?:unknown;profile?:unknown}={}):Promise<{id:string;url:string;title:string;engine:BrowserEngine;profile:BrowserProfile;width:number;height:number;external:boolean}>{
  ...
  const context=await browser.newContext(contextOptions),page=await context.newPage(),session:BrowserSession={id,browser,context,page,events:[],width:size.width,height:size.height,destroyed:false,browserClosed:false,headless,recording:true,snapshotStatus:'idle',snapshotVersion:0,snapshotGeneration:0,checkpointBusy:false,sticky:new Map(),navigationVersion:0,engine,profile,url:url.href,title:''};sessions.set(id,session);
  ...
  try{await page.goto(url.href,{waitUntil:'domcontentloaded',timeout:30000});session.url=page.url()||url.href;session.title=await safeTitle(page);await forceRuntimeScan(session);if(!headless)await page.bringToFront();scheduleCheckpoint(session,350);}catch(error){pushEvent(session,{source:'animator-preview',type:'DIAGNOSTIC',level:'warn',message:error instanceof Error?error.message:String(error)});scheduleCheckpoint(session,350);}
  return{id,url:session.url,title:session.title,engine,profile,width:size.width,height:size.height,external:!headless};
}
...
export function listBrowserSessionStates():BrowserSessionState[]{return [...sessions.values()].filter(session=>!session.destroyed).map(stateOf);}
export async function browserState(id:string):Promise<BrowserSessionState>{const session=requireSession(id);if(!session.browserClosed)await refreshSessionTitle(session);return stateOf(session);}
...
export async function hardStopBrowserSession(id:string,reason='control-hard-stop'):Promise<void>{const session=requireSession(id);markStoppedImmediately(session,reason);session.snapshotGeneration++;clearCheckpoint(session);if(session.snapshotHtml){session.snapshotStatus='ready';session.snapshotError=undefined;}else{session.snapshotStatus='error';session.snapshotError='Hard stop used before a browser checkpoint was available';}void closeBrowserWindow(id,reason);}
export async function closeBrowserWindow(id:string,reason='browser-close-command'):Promise<void>{const session=requireSession(id);if(session.browserClosed)return;if(session.recording)markStoppedImmediately(session,reason);session.browserClosed=true;session.snapshotGeneration++;clearCheckpoint(session);if(session.snapshotHtml){session.snapshotStatus='ready';session.snapshotError=undefined;}else if(session.snapshotStatus!=='ready'){session.snapshotStatus='error';session.snapshotError='Browser closed before a capture checkpoint was available';}try{await session.context.close();}catch{}try{await session.browser.close();}catch{}}
export async function closeBrowserSession(id:string):Promise<void>{const session=sessions.get(id);if(!session)return;sessions.delete(id);session.destroyed=true;session.browserClosed=true;session.snapshotGeneration++;clearCheckpoint(session);try{await session.context.close();}catch{}try{await session.browser.close();}catch{}}
export async function closeAllBrowserSessions():Promise<void>{await Promise.all([...sessions.keys()].map(closeBrowserSession));}

function stateOf(session:BrowserSession):BrowserSessionState{return{id:session.id,url:session.url,title:session.title,width:session.width,height:session.height,engine:session.engine,profile:session.profile,external:!session.headless,recording:session.recording,closed:session.browserClosed,snapshotReady:!session.recording&&session.snapshotStatus==='ready'&&!!session.snapshotHtml,snapshotStatus:session.snapshotStatus,snapshotVersion:session.snapshotVersion,checkpointReady:!!session.snapshotHtml,...(session.snapshotError?{snapshotError:session.snapshotError}:{})};}
...
async function applyBrowserCommand(session:BrowserSession,command:BrowserCommand):Promise<void>{
  ...
  if(type==='SET_RECORDING'){await setBrowserRecording(session,message);return;}
  if(session.browserClosed)throw new Error('Browser window is closed');
...
async function setBrowserRecording(session:BrowserSession,message:BrowserCommand):Promise<void>{
  ...
  if(enabled){session.snapshotHtml=undefined;session.snapshotStatus='idle';pushRecordingState(session,true,requestId,reason);void applyRecordingToPage(session,true,message.requestedAt).catch(error=>pushDiagnostic(session,'warn',`REC runtime apply failed: ${errorMessage(error)}`));scheduleCheckpoint(session,350);return;}
  session.snapshotStatus=session.snapshotHtml?'ready':'capturing';pushRecordingState(session,false,requestId,reason);const generation=session.snapshotGeneration;void finalizeStop(session,generation);
}
async function finalizeStop(session:BrowserSession,generation:number):Promise<void>{
  const previous=session.snapshotHtml;
  try{
    const html=await buildBrowserSnapshot(session,generation,64);
    if(!html||!captureCurrent(session,generation)||session.recording)return;
    session.snapshotHtml=html;session.snapshotStatus='ready';session.snapshotVersion++;session.snapshotError=undefined;
  }catch(error){if(!captureCurrent(session,generation)||session.recording)return;if(previous){session.snapshotHtml=previous;session.snapshotStatus='ready';session.snapshotError=undefined;}else{session.snapshotStatus='error';session.snapshotError=errorMessage(error);}pushDiagnostic(session,'warn',`Capture finalization failed: ${errorMessage(error)}`);}
  finally{if(captureCurrent(session,generation)&&!session.recording&&!session.browserClosed)void applyRecordingToPage(session,false).catch(error=>pushDiagnostic(session,'warn',`STOP runtime freeze failed: ${errorMessage(error)}`));}
}
async function applyRecordingToPage(session:BrowserSession,enabled:boolean,requestedAt?:unknown):Promise<void>{
  if(session.browserClosed||session.destroyed)return;
  if(enabled)await postToPage(session.page,{source:'animator-timeline',type:'RELEASE_TIMELINE'});
  await postToPage(session.page,{source:'animator-editor',type:'SET_RECORDING',enabled,requestedAt:Number(requestedAt)||Date.now()});
}
function markStoppedImmediately(session:BrowserSession,reason:string):void{if(!session.recording)return;session.recording=false;pushRecordingState(session,false,undefined,reason);}
function pushRecordingState(session:BrowserSession,enabled:boolean,requestId?:string,reason='command'):void{pushEvent(session,{source:'animator-preview',type:'RECORDING_STATE',enabled,...(requestId?{requestId}:{}),reason});}
function handleBrowserClosed(session:BrowserSession,reason:string):void{if(session.destroyed||session.browserClosed)return;session.browserClosed=true;session.snapshotGeneration++;clearCheckpoint(session);if(session.recording)markStoppedImmediately(session,reason);if(session.snapshotHtml){session.snapshotStatus='ready';session.snapshotError=undefined;}else if(session.snapshotStatus!=='ready'){session.snapshotStatus='error';session.snapshotError='Browser closed before a capture checkpoint was available';}pushDiagnostic(session,'info','Browser window closed; recording finalized automatically');}
function scheduleCheckpoint(session:BrowserSession,delay=5000):void{clearCheckpoint(session);if(session.destroyed||session.browserClosed||!session.recording)return;const generation=session.snapshotGeneration;session.checkpointTimer=setTimeout(()=>{session.checkpointTimer=undefined;void captureCheckpoint(session,generation);},delay);}
function clearCheckpoint(session:BrowserSession):void{if(session.checkpointTimer)clearTimeout(session.checkpointTimer);session.checkpointTimer=undefined;}
async function captureCheckpoint(session:BrowserSession,generation:number):Promise<void>{if(session.checkpointBusy||!captureCurrent(session,generation)||!session.recording)return;session.checkpointBusy=true;try{const html=await buildBrowserSnapshot(session,generation,16);if(html&&captureCurrent(session,generation)&&session.recording){session.snapshotHtml=html;session.snapshotVersion++;}}catch{}finally{session.checkpointBusy=false;if(captureCurrent(session,generation)&&session.recording)scheduleCheckpoint(session,5000);}}
...
async function buildBrowserSnapshot(session:BrowserSession,generation:number,visualLimit:number):Promise<string|undefined>{
...
function requireSession(id:string):BrowserSession{const session=sessions.get(id);if(!session||session.destroyed)throw new Error('Browser preview session not found');return session;}
