...
export async function scanBrowserMotion(id:string):Promise<BrowserMotionSnapshot>{
  ...
  return session.page.evaluate(()=>{
    ...
    const rawAnimations=(()=>{try{return document.getAnimations({subtree:true});}catch{return document.getAnimations();}})().slice(0,600);
    const animations:BrowserMotionAnimation[]=[];
    for(const animation of rawAnimations){
      const target=animation.effect?.target;if(!(target instanceof Element)||target.hasAttribute('data-animator-internal'))continue;
      ...
      let timing:ComputedEffectTiming={} as ComputedEffectTiming,rawFrames:ComputedKeyframe[]=[];try{timing=animation.effect?.getComputedTiming?.()??timing;rawFrames=animation.effect?.getKeyframes?.()??[];}catch{}
      ...
      const names=[...new Set(keyframes.flatMap(frame=>Object.keys(frame).filter(key=>!['offset','easing','composite','computedOffset'].includes(key))))];
      const properties=names.map(name=>({name,values:keyframes.map(frame=>frame[name]).filter(value=>value!=null).map(String)}));
      const cssAnimation=typeof CSSAnimation!=='undefined'&&animation instanceof CSSAnimation,cssTransition=typeof CSSTransition!=='undefined'&&animation instanceof CSSTransition;
      const type=cssAnimation?'css-animation':cssTransition?'css-transition':'web-animation';const name=cssAnimation?(animation as CSSAnimation).animationName:cssTransition?(animation as CSSTransition).transitionProperty:undefined;
