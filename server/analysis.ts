...
interface StaticAnimation {
  id:string; elementId:string; type:'css-animation'; name?:string; startTime:number; duration?:number; delay?:number;
  iterations?:number; direction?:string; easing?:string; fill?:string; properties:Array<{name:string;values:string[]}>;
  source:SourceRef; confidence:'exact'; runtimeState:'idle';
}
    ...
    item.root.walkRules((rule:Rule)=>{
      ...
      if(transitionDeclaration){
        ...
        transitions.push({
          ...
          properties:parseTransitionProperties(declarations),
          source:{file:item.rel,...(location?.line?{line:location.line}:{}),...(location?.column?{column:location.column}:{}),selector:rule.selector,snippet:transitionDeclaration.toString(),...(media?{media}: {})}
        });
      }
    });
  }

  for(const file of files.filter(file=>/\.(?:m?[jt]s|[jt]sx)$/i.test(file))){
    analyzeScript(file,project.root,candidates);
  }
...
function collectFiles(root:string):string[]{
  const out:string[]=[];
  const visit=(dir:string):void=>{for(const entry of fs.readdirSync(dir,{withFileTypes:true})){if(ignored.has(entry.name))continue;const full=path.join(dir,entry.name);if(entry.isDirectory())visit(full);else out.push(full);}};
  visit(root);return out;
}

function parseCssFiles(files:string[],root:string):ParsedCss[]{
  const out:ParsedCss[]=[];
  for(const file of files.filter(candidate=>/\.(?:css|scss)$/i.test(candidate))){
    const source=fs.readFileSync(file,'utf8');
    try{out.push({file,rel:path.relative(root,file).split(path.sep).join('/'),root:postcss.parse(source,{from:file})});}catch{/* Unsupported syntax remains visible through diagnostics at higher layers. */}
  }
  return out;
}

function directDeclarations(rule:Rule):Map<string,Declaration>{
  const map=new Map<string,Declaration>();
  for(const node of rule.nodes??[])if(node.type==='decl')map.set(node.prop.toLowerCase(),node);
  return map;
}
...
function parseAnimationShorthand(value:string,keyframes:Map<string,KeyframeRef>):AnimationSpec{
  ...
  for(const token of tokens){
    const time=toMs(token);
    if(time!==undefined){if(timeCount===0)spec.duration=time;else if(timeCount===1)spec.delay=time;timeCount++;continue;}
    if(isTiming(token)){spec.easing=token;continue;}
    if(token==='infinite'){continue;}
    const iterations=parseIterations(token);if(iterations!==undefined&&/^\d/.test(token)){spec.iterations=iterations;continue;}
    if(directions.has(token)){spec.direction=token;continue;}
    if(fills.has(token)){spec.fill=token;continue;}
    if(token==='running'||token==='paused')continue;
    if(keyframes.has(token)){spec.name=token;continue;}
    if(!animationReserved.has(token))unclassified.push(token);
  }
  if(!spec.name&&unclassified.length)spec.name=unclassified.at(-1);
  if(value.split(/\s+/).includes('infinite'))spec.iterations=undefined;
  return spec;
}

function parseTransitionProperties(declarations:Map<string,Declaration>):string[]{
  const explicit=declarations.get('transition-property')?.value;
  if(explicit)return cssList(explicit).filter(Boolean);
  const shorthand=declarations.get('transition')?.value;
  if(!shorthand)return['all'];
  return splitCssList(shorthand).map(part=>{
    for(const token of splitWhitespace(part))if(toMs(token)===undefined&&!isTiming(token)&&!['allow-discrete','normal'].includes(token))return token;
    return'all';
  });
}
...
function splitWhitespace(value:string):string[]{
  const out:string[]=[];let current='',depth=0,quote='';
  const flush=()=>{if(current.trim())out.push(current.trim());current='';};
  for(const char of value){if(quote){current+=char;if(char===quote)quote='';continue;}if(char==='"'||char==="'"){quote=char;current+=char;continue;}if(char==='('||char==='['){depth++;current+=char;continue;}if(char===')'||char===']'){depth=Math.max(0,depth-1);current+=char;continue;}if(/\s/.test(char)&&depth===0){flush();continue;}current+=char;}flush();return out;
}
...
function isInsideKeyframes(rule:Rule):boolean{let parent=rule.parent;while(parent){if(parent.type==='atrule'&&/(?:^|-)keyframes$/i.test(parent.name))return true;parent=parent.parent;}return false;}
function mediaContext(rule:Rule):string|undefined{const media:string[]=[];let parent=rule.parent;while(parent){if(parent.type==='atrule'&&parent.name.toLowerCase()==='media')media.unshift(parent.params);parent=parent.parent;}return media.length?media.join(' and '):undefined;}

function analyzeScript(file:string,root:string,candidates:StaticCandidate[]):void{
  const source=fs.readFileSync(file,'utf8');const rel=path.relative(root,file).split(path.sep).join('/');const kind=scriptKind(file);const sf=ts.createSourceFile(file,source,ts.ScriptTarget.Latest,true,kind);
  const scan=(node:ts.Node):void=>{
    ...
    if(ts.isCallExpression(node)){
      ...
      const left=node.left.getText(sf);if(/\.style\.(?:transform|opacity|left|top|right|bottom|width|height|filter|backgroundColor|borderRadius)$/.test(left))motionKind='style-write';
    }
...
function isMotionProperty(value:string):boolean{return motionFields.has(value.replace(/-([a-z])/g,(_,letter:string)=>letter.toUpperCase()).toLowerCase())||['transform','opacity','left','top','right','bottom','width','height','filter','background-color','border-radius'].includes(value.toLowerCase());}
function enclosingFunctionName(node:ts.Node,sf:ts.SourceFile):string|undefined{
  ...
  while(current){
    ...
    if((ts.isArrowFunction(current)||ts.isFunctionExpression(current))&&current.parent){const parent=current.parent;if(ts.isVariableDeclaration(parent)&&ts.isIdentifier(parent.name))return parent.name.text;if(ts.isPropertyAssignment(parent))return parent.name.getText(sf);if(ts.isCallExpression(parent)&&/addEventListener$/.test(parent.expression.getText(sf))){const eventArg=parent.arguments[0];return eventArg&&ts.isStringLiteralLike(eventArg)?`event:${eventArg.text}`:'event-callback';}}
