export const auxiliaryRuntimeSource = String.raw`(()=>{
  ...
  const SOURCE='animator-preview';let loop=false,lastFrame=performance.now(),lastDrop=0,lastConcurrent=0;
  const post=(type,payload={})=>parent.postMessage({source:SOURCE,type,...payload},'*');
  const event=(kind,label,data={})=>post('EVENT',{event:{id:'ev-'+Math.random().toString(36).slice(2),at:performance.now(),kind,label,data}});
  ...
  document.addEventListener('animationend',()=>{if(loop)queueMicrotask(()=>{for(const animation of document.getAnimations()){try{if(animation.playState==='finished'){animation.currentTime=0;animation.play();}}catch{}}});},true);
