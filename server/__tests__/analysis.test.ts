...
describe('analysis',()=>{
  it('finds CSS and JS motion',()=>{const result=analyzeProject({id:'fixture',root,entries:['index.html'],selectedEntry:'index.html',tree:[]});expect(result.animations.length).toBeGreaterThan(0);expect(result.transitions.length).toBeGreaterThan(0);expect(result.candidates.some(x=>x.kind==='waapi')).toBe(true);expect(result.candidates.some(x=>x.kind==='raf')).toBe(true);expect(result.reducedMotion).toBe(true);});
  it('resolves multiple animations across files and media context',()=>{
    ...
    expect(result.animations[0]?.source.media).toContain('max-width');
