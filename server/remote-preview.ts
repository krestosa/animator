...
function parseRemoteUrl(input:string):URL{const raw=input.trim();if(!raw)throw new Error('Enter a web URL');const value=/^https?:\/\//i.test(raw)?raw:'https://'+raw,url=new URL(value);if(!['http:','https:'].includes(url.protocol))throw new Error('Only http and https URLs are supported');if(url.username||url.password)throw new Error('URLs with embedded credentials are not supported');return url;}
export function mapRemoteRedirect(location:string,target:URL,localOrigin:string):{remote:URL;local:string}{const remote=new URL(location,target);return{remote,local:localOrigin+remote.pathname+remote.search+remote.hash};}
...
function appendTheme(input:string,mode:PreviewColorScheme):string{const url=new URL(input);url.searchParams.delete('__animator_color_scheme_reset');if(mode==='auto'||mode==='system')url.searchParams.set('__animator_color_scheme_reset','1');else url.searchParams.set('__animator_color_scheme',mode);return url.toString();}
