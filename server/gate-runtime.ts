export const gateRuntimeSource=String.raw`(()=>{
  ...
  const reportAnimation=animation=>{
    if(!recording||!animation||animation.__animatorMirror||animation.__animatorMutationMirror)return;
    const el=animation?.effect?.target,idFor=window.__ANIMATOR_ELEMENT_ID__;if(!(el instanceof Element)||typeof idFor!=='function'||el.hasAttribute('data-animator-internal'))return;
  ...
  const reportActiveAnimations=()=>{if(!recording)return;for(const animation of document.getAnimations?.()??[])reportAnimation(animation);};
  const resumeAnimations=requestedAt=>{
    ...
    for(const animation of rescued)reportAnimation(animation);
  };
