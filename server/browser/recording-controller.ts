...
const minimumBackgroundCheckpointDelay=15000;
const recurringBackgroundCheckpointDelay=30000;
...
export function scheduleCheckpoint(session:BrowserSession,hooks:RecordingHooks,delay=recurringBackgroundCheckpointDelay):void{clearCheckpoint(session);if(session.destroyed||session.browserClosed||!session.recording)return;const generation=session.snapshotGeneration,effectiveDelay=Math.max(minimumBackgroundCheckpointDelay,Number(delay)||recurringBackgroundCheckpointDelay);session.checkpointTimer=setTimeout(()=>{session.checkpointTimer=undefined;void captureCheckpoint(session,generation,hooks);},effectiveDelay);}
...
async function captureCheckpoint(session:BrowserSession,generation:number,hooks:RecordingHooks):Promise<void>{if(session.checkpointBusy||!captureCurrent(session,generation)||!session.recording)return;session.checkpointBusy=true;try{const html=await buildBrowserSnapshot(session,generation,2);if(html&&captureCurrent(session,generation)&&session.recording){session.snapshotHtml=html;session.snapshotVersion++;}}catch{}finally{session.checkpointBusy=false;if(captureCurrent(session,generation)&&session.recording)scheduleCheckpoint(session,hooks,recurringBackgroundCheckpointDelay);}}
