export const mutationRuntimeSource = String.raw`(()=>{
  ...
  window.__ANIMATOR_MUTATION_REPLAY__={seek:replay,release,tracks:tracksById};
  ...
  const showTarget=id=>{const target=targetFor(id);if(!(target instanceof Element))return;const meta=elementMeta(target);post('ELEMENTS',{elements:[meta]});post('SELECT_ELEMENT',{element:meta});const rect=target.getBoundingClientRect();if(!highlight){highlight=document.createElement('div');Object.assign(highlight.style,{position:'fixed',pointerEvents:'none',zIndex:'2147483645',border:'2px solid #d4a44c',background:'rgba(212,164,76,.08)',boxSizing:'border-box',transition:'opacity .15s'});document.documentElement.appendChild(highlight);}Object.assign(highlight.style,{display:'block',opacity:'1',left:rect.left+'px',top:rect.top+'px',width:rect.width+'px',height:rect.height+'px'});setTimeout(()=>{if(highlight)highlight.style.opacity='0';},900);};
  addEventListener('message',event=>{const m=event.data;if(!m||m.source!==IN||typeof m.type!=='string')return;if(m.type==='SCRUB_TIMELINE')replay(Math.max(0,Number(m.time)||0));else if(m.type==='PLAY_ALL'||m.type==='PAUSE_ALL'||m.type==='RESTART_ALL')controlled=true;else if(m.type==='RELEASE_TIMELINE'||m.type==='CLEAR_OVERRIDES')release();else if(m.type==='HIGHLIGHT_ANIMATION')showTarget(m.id);});
})();`;
