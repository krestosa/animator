...
export const recordResumeRuntimeSource=themeLiveRuntimeSource+viewHistoryRuntimeSource+String.raw`(()=>{
  ...
  addEventListener('message',event=>{const message=event.data;if(!message||message.source!=='animator-editor'||message.type!=='SET_RECORDING'||!message.enabled)return;queueMicrotask(scan);requestAnimationFrame(scan);});
})();`;
