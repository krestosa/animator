...
import { browserFrame, browserInput, browserState, closeAllBrowserSessions, closeBrowserSession, drainBrowserEvents, openBrowserSession, sendBrowserCommand } from './browser-session.js';
...
app.post('/api/browser-sessions/open',async(req,res)=>{try{return res.json(await openBrowserSession(String(req.body?.url||''),Number(req.body?.width)||1100,Number(req.body?.height)||700));}catch(error){return res.status(400).json({error:error instanceof Error?error.message:String(error)});}});
