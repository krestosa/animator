...
import { browserSnapshot, browserState, closeAllBrowserSessions, closeBrowserSession, closeBrowserWindow, drainBrowserEvents, hardStopBrowserSession, installBrowserRuntimes, listBrowserRuntimes, listBrowserSessionStates, openBrowserSession, sendBrowserCommand, type BrowserSessionState } from './browser-session.js';
...
let activeBrowserSessionId='',controlRevision=0;
const bumpControl=(sessionId?:string):number=>{if(sessionId!==undefined)activeBrowserSessionId=sessionId;return++controlRevision;};
const resolveControlSession=(value:unknown):string=>{const requested=String(value||'').trim();if(requested)return requested;if(activeBrowserSessionId)return activeBrowserSessionId;const sessions=listBrowserSessionStates();return sessions.find(session=>session.recording)?.id??sessions.at(-1)?.id??'';};
const browserProject=(session:{id:string;url:string;engine:string;profile:string})=>({id:session.id,root:session.url,entries:['/'],selectedEntry:'/',tree:[],sourceUrl:session.url,kind:'remote' as const,browserSessionId:session.id,browserEngine:session.engine,browserProfile:session.profile});
const openControlledBrowser=async(body:Record<string,unknown>)=>{const profile=body.profile==='mobile'?'mobile':'desktop',session=await openBrowserSession(String(body.url||''),{width:Number(body.width)||(profile==='mobile'?390:1100),height:Number(body.height)||(profile==='mobile'?844:700),engine:body.engine,profile});bumpControl(session.id);return session;};
const controlStatus=()=>{const sessions=listBrowserSessionStates(),active=sessions.find(session=>session.id===activeBrowserSessionId)??null;if(activeBrowserSessionId&&!active)activeBrowserSessionId='';return{revision:controlRevision,activeSessionId:activeBrowserSessionId||null,activeSession:active,sessions};};
...
app.post('/api/browser-sessions/open',async(req,res)=>{try{return res.json(await openControlledBrowser(req.body as Record<string,unknown>));}catch(error){return res.status(400).json({error:error instanceof Error?error.message:String(error)});}});
...
app.delete('/api/browser-sessions/:id',async(req,res)=>{await closeBrowserSession(req.params.id);if(activeBrowserSessionId===req.params.id)bumpControl('');return res.status(204).end();});

app.get('/api/control/status',(_req,res)=>res.json(controlStatus()));
app.post('/api/control/open',async(req,res)=>{try{const session=await openControlledBrowser(req.body as Record<string,unknown>);return res.json({ok:true,revision:controlRevision,session,project:browserProject(session)});}catch(error){return res.status(400).json({error:error instanceof Error?error.message:String(error)});}});
app.post('/api/control/record',async(req,res)=>{try{const id=resolveControlSession(req.body?.sessionId);if(!id)return res.status(404).json({error:'No active browser session'});const enabled=req.body?.enabled!==false,requestId=`control-${Date.now()}-${Math.random().toString(36).slice(2,8)}`;await sendBrowserCommand(id,{type:'SET_RECORDING',enabled,requestId,reason:'control'});bumpControl(id);return res.json({ok:true,revision:controlRevision,state:await browserState(id)});}catch(error){return res.status(409).json({error:error instanceof Error?error.message:String(error)});}});
app.post('/api/control/stop',async(req,res)=>{try{const id=resolveControlSession(req.body?.sessionId);if(!id)return res.status(404).json({error:'No active browser session'});if(req.body?.hard===true)await hardStopBrowserSession(id,'control-hard-stop');else await sendBrowserCommand(id,{type:'SET_RECORDING',enabled:false,requestId:`control-${Date.now()}-${Math.random().toString(36).slice(2,8)}`,reason:'control-stop'});bumpControl(id);return res.json({ok:true,revision:controlRevision,state:await browserState(id)});}catch(error){return res.status(409).json({error:error instanceof Error?error.message:String(error)});}});
app.post('/api/control/start',async(req,res)=>{try{const id=resolveControlSession(req.body?.sessionId);if(!id)return res.status(404).json({error:'No active browser session'});await sendBrowserCommand(id,{type:'SET_RECORDING',enabled:true,requestId:`control-${Date.now()}-${Math.random().toString(36).slice(2,8)}`,reason:'control-start'});bumpControl(id);return res.json({ok:true,revision:controlRevision,state:await browserState(id)});}catch(error){return res.status(409).json({error:error instanceof Error?error.message:String(error)});}});
app.post('/api/control/close',async(req,res)=>{try{const id=resolveControlSession(req.body?.sessionId);if(!id)return res.status(404).json({error:'No active browser session'});await closeBrowserWindow(id,'control-close');bumpControl(id);return res.json({ok:true,revision:controlRevision,state:await browserState(id)});}catch(error){return res.status(409).json({error:error instanceof Error?error.message:String(error)});}});
app.post('/api/control/browsers/install',async(req,res)=>{try{return res.json({ok:true,runtimes:await installBrowserRuntimes(req.body?.engines)});}catch(error){return res.status(500).json({error:error instanceof Error?error.message:String(error)});}});
