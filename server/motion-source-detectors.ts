...
function readObject(node:ts.Node|undefined,sf:ts.SourceFile):Record<string,unknown>|undefined{if(!node||!ts.isObjectLiteralExpression(node))return undefined;const out:Record<string,unknown>={};for(const prop of node.properties){if(!ts.isPropertyAssignment(prop)&&!ts.isShorthandPropertyAssignment(prop))continue;const key=prop.name.getText(sf).replace(/^['"]|['"]$/g,'');if(ts.isShorthandPropertyAssignment(prop)){out[key]=resolveShorthand(prop.name.text,sf);continue;}out[key]=literalValue(prop.initializer,sf);}return out;}
function resolveShorthand(name:string,sf:ts.SourceFile):unknown{let initializer:ts.Expression|undefined;const scan=(node:ts.Node):void=>{if(initializer)return;if(ts.isVariableDeclaration(node)&&ts.isIdentifier(node.name)&&node.name.text===name&&node.initializer){initializer=node.initializer;return;}ts.forEachChild(node,scan);};scan(sf);return initializer?literalValue(initializer,sf):name;}
