export const seekRuntimeSource = String.raw`(()=>{
  ...
  const applyClock=()=>{if(!playing)return;const perf=performance.now();if(perf-lastAppliedPerf<12)return;lastAppliedPerf=perf;let next=playOriginTime+(perf-playOriginPerf)*rate;if(loop&&endCache>0&&next>endCache)next%=endCache;masterTime=Math.max(0,next);masterFrame=Math.round(masterTime/FRAME_MS);seekAll(masterTime);postState(false);};
