export const seekRuntimeSource = String.raw`(()=>{
  ...
  const nativeRAF=window.requestAnimationFrame.bind(window),nativeAnimate=Element.prototype.animate;
  let controlled=false,playing=false,masterTime=0,rate=1,loop=false,lastPerf=performance.now(),raf=0,highlight=null;
  ...
  const ensureMirror=animation=>{
    ...
    try{
      animation.pause();
      const effect=new KeyframeEffect(target,info.frames,info.timing);
      const mirror=new Animation(effect,document.timeline);mirror.__animatorMirror=true;mirror.pause();
      mirrors.set(animation,mirror);return mirror;
  ...
  const seekOne=(animation,time)=>{
    ...
    try{
      controller.pause();
      const end=endOf(controller);
      controller.currentTime=Number.isFinite(end)?Math.min(local,end):local;
    }catch{}
  };
  const seekAll=time=>{
    for(const animation of all())seekOne(animation,time);
    void document.documentElement.getBoundingClientRect();
    for(const animation of all()){const target=animation.effect?.target;if(target instanceof Element){void getComputedStyle(target).transform;void getComputedStyle(target).opacity;}}
  };
  const totalEnd=()=>{let end=0;for(const animation of all()){const info=remember(animation),duration=endOf(animation);if(Number.isFinite(duration))end=Math.max(end,info.start+duration);}return end;};
  ...
  const pause=()=>{if(!controlled)enter(masterTime);playing=false;if(raf){cancelAnimationFrame(raf);raf=0;}seekAll(masterTime);post('TIMELINE_STATE',{time:masterTime,playing,controlled});};
  ...
  const restore=()=>{
    ...
    controlled=false;post('TIMELINE_STATE',{time:masterTime,playing:false,controlled:false});
  };
