...
export function groupAnimations(tracks:MotionTrack[],selectedId?:string):AnimationGroup[]{const grouped=new Map<string,MotionTrack[]>();for(const track of tracks){const key=animationGroupKey(track),items=grouped.get(key)??[];items.push(track);grouped.set(key,items);}return[...grouped].map(([key,instances])=>({key,instances,representative:instances.find(item=>item.id===selectedId)||bestRepresentative(instances)})).sort((a,b)=>Math.min(...a.instances.map(item=>item.timing.start))-Math.min(...b.instances.map(item=>item.timing.start)));}
...
function score(track:MotionTrack):number{return(track.source.confidence==='exact'?8:track.source.confidence==='source-correlated'?7:track.source.confidence==='runtime-observed'?6:2)+(track.keyframes.length?4:0)+(track.source.reference?3:0)+(track.target.elementId.startsWith('static:')?0:2);}
