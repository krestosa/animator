...
export interface AnimationGroup<T extends MotionTrack=MotionTrack>{key:string;representative:T;instances:T[];}
...
export function groupAnimations<T extends MotionTrack>(tracks:T[],selectedId?:string):AnimationGroup<T>[]{
  const grouped=new Map<string,T[]>();for(const track of tracks){const key=animationGroupKey(track),items=grouped.get(key)??[];items.push(track);grouped.set(key,items);}
...
function bestRepresentative<T extends MotionTrack>(items:T[]):T{return[...items].sort((a,b)=>score(b)-score(a))[0]??items[0]!;}
