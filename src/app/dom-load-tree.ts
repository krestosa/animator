...
export function mountDomLoadTree(root:HTMLElement):()=>void{
  ...
  let nodes:TimelineEvent[]=[],signature='',raf=0;
  const frame=()=>root.querySelector<HTMLIFrameElement>('[data-preview-frame]');
  const updateNodes=():void=>{const all=store.get().events.filter(event=>event.kind==='dom-build');const next=`${all.length}:${all.at(-1)?.id??''}`;if(next===signature)return;signature=next;nodes=all;canvas.style.height=`${nodes.length*ROW_HEIGHT}px`;const count=section.querySelector<HTMLElement>('[data-dom-count]');if(count)count.textContent=String(nodes.length);renderWindow();};
  ...
  scroller.addEventListener('scroll',scroll,{passive:true});section.addEventListener('click',click);const unsubscribe=store.subscribe(()=>{schedule();timeline();});schedule();
