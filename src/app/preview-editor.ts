...
export function mountPreviewEditor(root:HTMLElement):()=>void{
  ...
  top.append(controls);
  let captureTimer=0;let activeFrame:Window|null=null;let destroyed=false;let timelinePlaying=false;
  ...
  const seek=(time:number):void=>{const next=Math.max(0,time);timelinePlaying=false;store.set({playhead:next});sendCommand(iframe(),{type:'SCRUB_TIMELINE',time:next});};
  ...
  const recapture=():void=>{
    ...
    timelinePlaying=false;if(captureTimer)window.clearTimeout(captureTimer);
  ...
  const message=(event:MessageEvent<unknown>):void=>{
    ...
    const data=event.data as {source?:unknown;type?:unknown;playing?:unknown};if(data.source!=='animator-preview')return;
    if(data.type==='TIMELINE_STATE'){timelinePlaying=data.playing===true;return;}
