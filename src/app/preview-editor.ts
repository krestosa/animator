...
export function mountPreviewEditor(root:HTMLElement):()=>void{
  ...
  top.append(controls);
  let captureTimer=0,activeFrame:Window|null=null,destroyed=false,timelinePlaying=false,controlled=false,currentTime=0,currentFrame=0,autoCapture=true,lastReport:CaptureReportMessage|undefined;
  ...
  const updateMode=():void=>{const button=liveButton();if(button){button.classList.toggle('active',!controlled);button.textContent=controlled?'Live':'Live ●';button.title=controlled?'Release timeline and resume live viewport capture':'Live capture active';}};
  ...
  const reportText=(report:CaptureReportMessage):string=>!store.get().recording?'Recording paused · timeline frozen':`${report.burstActive?'Scanning':'Viewport'} · ${report.visibleElements} visible · ${report.activeAnimations} active · ${report.capturedAnimations} captured · ${report.styleTracks} JS`;
  ...
  const release=():void=>{timelinePlaying=false;controlled=false;updateMode();updateFrameLabel();sendCommand(iframe(),{type:'RELEASE_TIMELINE'});const label=status();if(label)label.textContent=!store.get().recording?'Recording paused · timeline frozen':lastReport?reportText(lastReport):'Live capture · scroll/interact with the page';};
  const recalculate=():void=>{const frame=iframe();if(!frame)return;const button=recalcButton();if(button)button.disabled=true;const label=status();if(label)label.textContent=store.get().recording?'Recalculating viewport…':'Recording paused · timeline frozen';sendCommand(frame,{type:'RECALCULATE_VIEWPORT'});window.setTimeout(()=>{if(button)button.disabled=false;},450);};
  const setAutoCapture=(enabled:boolean):void=>{autoCapture=enabled;sendCommand(iframe(),{type:'SET_AUTO_VIEWPORT_CAPTURE',enabled});const label=status();if(label)label.textContent=enabled?'Auto viewport capture enabled':'Auto viewport capture disabled';};
  ...
  const message=(event:MessageEvent<unknown>):void=>{
    ...
    activeFrame=event.source as Window;controlled=false;timelinePlaying=false;updateMode();updateFrameLabel();const recording=store.get().recording;const label=status();if(label)label.textContent=recording?'Capturing startup + viewport motion…':'Recording paused · timeline frozen';if(captureTimer)window.clearTimeout(captureTimer);sendCommand(frame,{type:'SET_RECORDING',enabled:recording});sendCommand(frame,{type:'SET_AUTO_VIEWPORT_CAPTURE',enabled:autoCapture});
    captureTimer=window.setTimeout(()=>{captureTimer=0;if(destroyed||activeFrame!==iframe()?.contentWindow)return;const runtimeCount=store.get().animations.filter(animation=>!animation.elementId.startsWith('static:')).length;const current=status();if(current)current.textContent=!store.get().recording?'Recording paused · timeline frozen':lastReport?reportText(lastReport):`Live ready · ${runtimeCount} runtime motion${runtimeCount===1?'':'s'} · ${FPS} fps`;},1400);
  };
  updateMode();updateFrameLabel();controls.addEventListener('click',click);controls.addEventListener('change',change);window.addEventListener('keydown',key);window.addEventListener('message',message);window.addEventListener(TIMELINE_STATE_EVENT,timelineState);window.addEventListener(CAPTURE_REPORT_EVENT,captureReport);
  return()=>{destroyed=true;if(captureTimer)window.clearTimeout(captureTimer);controls.removeEventListener('click',click);controls.removeEventListener('change',change);window.removeEventListener('keydown',key);window.removeEventListener('message',message);window.removeEventListener(TIMELINE_STATE_EVENT,timelineState);window.removeEventListener(CAPTURE_REPORT_EVENT,captureReport);controls.remove();};
}