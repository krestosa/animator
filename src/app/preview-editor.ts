...
export function mountPreviewEditor(root:HTMLElement):()=>void{
  ...
  const controls=document.createElement('div');controls.className='previewEditorControls';controls.innerHTML='<button data-preview-live title="Release timeline and interact with the live page">Live</button><button data-preview-frame-back title="Previous frame">−1f</button><button data-preview-frame-forward title="Next frame">+1f</button><button data-preview-recapture title="Reload page and capture startup animations">Recapture</button><span data-preview-capture-status>Waiting for preview</span>';
  top.append(controls);
  let captureTimer=0;let activeFrame:Window|null=null;let destroyed=false;let timelinePlaying=false;let controlled=false;
  ...
  const seek=(time:number):void=>{const next=Math.max(0,time);timelinePlaying=false;controlled=true;updateMode();store.set({playhead:next});sendCommand(iframe(),{type:'SCRUB_TIMELINE',time:next});};
  ...
  const recapture=():void=>{
    ...
    timelinePlaying=false;controlled=false;updateMode();if(captureTimer)window.clearTimeout(captureTimer);
  ...
  const click=(event:MouseEvent):void=>{const target=(event.target as Element|null)?.closest<HTMLElement>('[data-preview-live],[data-preview-frame-back],[data-preview-frame-forward],[data-preview-recapture]');if(!target)return;if(target.hasAttribute('data-preview-live'))release();else if(target.hasAttribute('data-preview-frame-back'))step(-1);else if(target.hasAttribute('data-preview-frame-forward'))step(1);else recapture();};
  const key=(event:KeyboardEvent):void=>{
    const target=event.target as HTMLElement|null;if(target?.matches('input,textarea,select,[contenteditable="true"]'))return;
    if(event.key==='ArrowLeft'){event.preventDefault();step(event.shiftKey?-10:-1);}else if(event.key==='ArrowRight'){event.preventDefault();step(event.shiftKey?10:1);}else if(event.key===' '&&!event.repeat){event.preventDefault();sendCommand(iframe(),{type:timelinePlaying?'PAUSE_ALL':'PLAY_ALL'});}
  };
  const message=(event:MessageEvent<unknown>):void=>{
    ...
    const data=event.data as {source?:unknown;type?:unknown;playing?:unknown;controlled?:unknown};if(data.source!=='animator-preview')return;
    if(data.type==='TIMELINE_STATE'){timelinePlaying=data.playing===true;controlled=data.controlled===true;updateMode();return;}
  ...
  updateMode();controls.addEventListener('click',click);window.addEventListener('keydown',key);window.addEventListener('message',message);
