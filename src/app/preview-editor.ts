...
export function mountPreviewEditor(root:HTMLElement):()=>void{
  ...
  top.append(controls);
  let captureTimer=0,activeFrame:Window|null=null,destroyed=false,timelinePlaying=false,controlled=false,currentTime=0,currentFrame=0,autoCapture=true,lastReport:CaptureReportMessage|undefined,lastRecording=store.get().recording,recalculatePending=false;
  ...
  const recalculate=():void=>{if(!store.get().recording||recalculatePending)return;recalculatePending=true;const frame=iframe(),button=recalcButton();if(button)button.disabled=true;const label=status();if(label)label.textContent='Recalculating viewport…';sendCommand(frame,{type:'RECALCULATE_VIEWPORT'});};
  ...
  const recapture=():void=>{
    ...
    timelinePlaying=false;controlled=false;currentTime=0;currentFrame=0;lastReport=undefined;recalculatePending=false;updateMode();updateFrameLabel();if(captureTimer)window.clearTimeout(captureTimer);
