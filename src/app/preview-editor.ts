...
export function mountPreviewEditor(root:HTMLElement):()=>void{
  ...
  const reportText=(report:CaptureReportMessage):string=>`${report.burstActive?'Scanning':'Viewport'} · ${report.visibleElements} visible · ${report.activeAnimations} active · ${report.capturedAnimations} captured · ${report.styleTracks} JS`;
  ...
  const release=():void=>{timelinePlaying=false;controlled=false;updateMode();updateFrameLabel();sendCommand(iframe(),{type:'RELEASE_TIMELINE'});const label=status();if(label)label.textContent=lastReport?reportText(lastReport):'Live capture · scroll/interact with the page';};
  const recalculate=():void=>{const frame=iframe();if(!frame)return;const button=recalcButton();if(button)button.disabled=true;const label=status();if(label)label.textContent='Recalculating viewport…';sendCommand(frame,{type:'RECALCULATE_VIEWPORT'});window.setTimeout(()=>{if(button)button.disabled=false;},450);};
  ...
  const message=(event:MessageEvent<unknown>):void=>{
    ...
    activeFrame=event.source as Window;controlled=false;timelinePlaying=false;updateMode();updateFrameLabel();const label=status();if(label)label.textContent='Capturing startup + viewport motion…';if(captureTimer)window.clearTimeout(captureTimer);sendCommand(frame,{type:'SET_AUTO_VIEWPORT_CAPTURE',enabled:autoCapture});
    captureTimer=window.setTimeout(()=>{captureTimer=0;if(destroyed||activeFrame!==iframe()?.contentWindow)return;const runtimeCount=store.get().animations.filter(animation=>!animation.elementId.startsWith('static:')).length;const current=status();if(current)current.textContent=lastReport?reportText(lastReport):`Live ready · ${runtimeCount} runtime motion${runtimeCount===1?'':'s'} · ${FPS} fps`;},1400);
  };
