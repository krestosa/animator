...
const RECALCULATE_TIMEOUT_MS=2500;
...
export function mountPreviewEditor(root:HTMLElement):()=>void{
  ...
  top.append(controls);
  let captureTimer=0,recalculateTimer=0,activeFrame:Window|null=null,destroyed=false,timelinePlaying=false,controlled=false,currentTime=0,currentFrame=0,autoCapture=true,lastReport:CaptureReportMessage|undefined,lastRecording=store.get().recording,recalculatePending=false;
  ...
  const clearRecalculateTimer=():void=>{if(recalculateTimer){window.clearTimeout(recalculateTimer);recalculateTimer=0;}};
  ...
  const syncRecordingUi=():void=>{const recording=store.get().recording,button=recalcButton();if(!recording){recalculatePending=false;clearRecalculateTimer();}if(button){button.disabled=!recording||recalculatePending||Boolean(lastReport?.burstActive);button.title=recording?'Rescan visible elements and capture viewport motion without reloading':'Start Record before recalculating viewport motion';}if(recording===lastRecording)return;lastRecording=recording;const label=status();if(label)label.textContent=recording?(lastReport?reportText(lastReport):'Recording active · waiting for runtime activity'):'Recording STOPPED · no new runtime capture';};
  ...
  const recalculate=():void=>{if(!store.get().recording||recalculatePending)return;recalculatePending=true;clearRecalculateTimer();const frame=iframe(),button=recalcButton();if(button)button.disabled=true;const label=status();if(label)label.textContent='Recalculating viewport…';sendCommand(frame,{type:'RECALCULATE_VIEWPORT'});recalculateTimer=window.setTimeout(()=>{recalculateTimer=0;if(destroyed||!recalculatePending)return;recalculatePending=false;syncRecordingUi();const current=status();if(current)current.textContent='Viewport recalculation timed out · retry available';},RECALCULATE_TIMEOUT_MS);};
  ...
  const recapture=():void=>{
    ...
    timelinePlaying=false;controlled=false;currentTime=0;currentFrame=0;lastReport=undefined;recalculatePending=false;clearRecalculateTimer();updateMode();updateFrameLabel();if(captureTimer)window.clearTimeout(captureTimer);
  ...
  const captureReport=(event:Event):void=>{const detail=(event as CustomEvent<CaptureReportMessage>).detail;if(!detail)return;lastReport=detail;if(!detail.burstActive){recalculatePending=false;clearRecalculateTimer();}autoCapture=detail.autoCapture;const toggle=controls.querySelector<HTMLInputElement>('[data-preview-auto]');if(toggle)toggle.checked=autoCapture;const label=status();if(label)label.textContent=reportText(detail);const button=recalcButton();if(button)button.disabled=!store.get().recording||recalculatePending||detail.burstActive;};
  const message=(event:MessageEvent<unknown>):void=>{
    ...
    activeFrame=event.source as Window;controlled=false;timelinePlaying=false;recalculatePending=false;clearRecalculateTimer();updateMode();updateFrameLabel();const recording=store.get().recording;const label=status();if(label)label.textContent=recording?'Capturing startup + viewport motion…':'Recording STOPPED · preview is live but not captured';if(captureTimer)window.clearTimeout(captureTimer);sendCommand(frame,{type:'SET_RECORDING',enabled:recording});sendCommand(frame,{type:'SET_AUTO_VIEWPORT_CAPTURE',enabled:autoCapture});
