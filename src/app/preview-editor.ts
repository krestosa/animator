...
export function mountPreviewEditor(root:HTMLElement):()=>void{
  ...
  const release=():void=>{timelinePlaying=false;controlled=false;updateMode();sendCommand(iframe(),{type:'RELEASE_TIMELINE'});const label=status();if(label)label.textContent=lastReport?reportText(lastReport):'Live capture · scroll/interact with the page';};
  ...
  const message=(event:MessageEvent<unknown>):void=>{
    ...
    activeFrame=event.source as Window;controlled=false;timelinePlaying=false;updateMode();const label=status();if(label)label.textContent='Capturing startup + viewport motion…';if(captureTimer)window.clearTimeout(captureTimer);sendCommand(frame,{type:'SET_AUTO_VIEWPORT_CAPTURE',enabled:autoCapture});
