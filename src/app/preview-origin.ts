...
export function mountPreviewOrigin(root:HTMLElement):()=>void{
  ...
  const apply=():void=>{
    ...
    const entry=project.selectedEntry.split('/').map(encodeURIComponent).join('/');const desired=`${project.previewOrigin.replace(/\/$/,'')}/${entry}`;
    ...
    if(currentFrame!==frame){currentFrame=frame;frame.addEventListener('load',()=>store.set({diagnostics:[...store.get().diagnostics,`info: Complete preview loaded from ${project.previewOrigin}`].slice(-100)}),{once:true});}
  };
