...
export function mountBrowserPreview(root:HTMLElement):()=>void{
  ...
  const applyDeviceSize=(width:number,height:number):void=>{const safeWidth=Math.max(1,width),safeHeight=Math.max(1,height),scale=Math.min(1,900/safeWidth,620/safeHeight);device.style.width=`${safeWidth}px`;device.style.height=`${safeHeight}px`;device.style.transform=`scale(${scale})`;};
