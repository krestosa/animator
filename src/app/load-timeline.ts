...
export function mountLoadTimeline(root:HTMLElement):()=>void{
  ...
  const changed=():void=>{const viewport=root.querySelector<HTMLElement>('[data-timeline-v2]');const pxPerMs=viewport?.dataset.pxPerMs;if(store.get().events!==lastEvents||!viewport?.querySelector('[data-load-timeline]')||!signature.startsWith(`${pxPerMs}|`))schedule();};
