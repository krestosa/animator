...
export function mountLoadTimeline(root:HTMLElement):()=>void{
  let raf=0,signature='';
  const render=():void=>{
    raf=0;const viewport=root.querySelector<HTMLElement>('[data-timeline-v2]');if(!viewport)return;
    const events=store.get().events,dom=events.filter(event=>event.kind==='dom-build'),network=events.filter(event=>event.kind==='network-resource'),lifecycle=events.filter(event=>event.kind==='document-lifecycle');
    const pxPerMs=Number(viewport.dataset.pxPerMs??.1);const next=`${pxPerMs}|${dom.length}:${dom.at(-1)?.id??''}|${network.length}:${network.at(-1)?.id??''}|${lifecycle.length}:${lifecycle.at(-1)?.id??''}`;if(next===signature&&viewport.querySelector('[data-load-timeline]'))return;signature=next;
  ...
  const schedule=():void=>{if(!raf)raf=requestAnimationFrame(()=>requestAnimationFrame(render));};
  const unsubscribe=store.subscribe(schedule);const observer=new MutationObserver(schedule);observer.observe(root,{subtree:true,childList:true});schedule();
  return()=>{unsubscribe();observer.disconnect();if(raf)cancelAnimationFrame(raf);root.querySelectorAll('[data-load-timeline]').forEach(node=>node.remove());};
}
