...
export function mountTimelineTools(root:HTMLElement):()=>void{
  ...
  let isolation:Isolation='all',raf=0,zoomTarget=store.get().zoom,focusMode=false,focusedAnimationId:string|undefined,isolatedElementId:string|undefined;
  ...
  const beginInspection=(animation:DetectedAnimation):void=>{
    const origin=animationOrigin(animation);focusedAnimationId=animation.id;isolatedElementId=animation.elementId;isolation='animation';
  ...
  const clearInspection=():void=>{
    leaveSoloInspection();isolatedElementId=undefined;isolation='all';applyIsolation();requestAnimationFrame(()=>centerPlayhead());
  };
  ...
  const inspected=(event:Event):void=>{const detail=(event as CustomEvent<InspectionDetail>).detail;if(!detail)return;focusedAnimationId=detail.id;isolatedElementId=detail.elementId;isolation='animation';applyIsolation();requestAnimationFrame(()=>viewport()?.scrollTo({left:0,behavior:'smooth'}));};
