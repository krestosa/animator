...
export function mountTimelineTools(root:HTMLElement):()=>void{
  ...
  let isolation:Isolation='all',raf=0,zoomTarget=store.get().zoom,focusMode=false,focusedAnimationId:string|undefined,isolatedAnimationId:string|undefined,isolatedElementId:string|undefined;
  ...
  return()=>{if(focusMode)setFocusMode(false);leaveSoloInspection();unsubscribe();observer.disconnect();if(raf)cancelAnimationFrame(raf);tools.removeEventListener('click',click);window.removeEventListener('keydown',key);window.removeEventListener(ANIMATION_INSPECT_EVENT,inspected);root.removeEventListener('wheel',wheel);tools.remove();};
}