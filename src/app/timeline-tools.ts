...
export function mountTimelineTools(root:HTMLElement):()=>void{
  ...
  const beginInspection=(animation:DetectedAnimation):void=>{
    pauseRecording();const origin=animationOrigin(animation);focusedAnimationId=animation.id;isolatedElementId=animation.elementId;isolation='animation';
  ...
  const setFocusMode=(enabled:boolean):void=>{
    if(enabled){const animation=selectedAnimation();if(!animation)return;pauseRecording();if(focusedAnimationId!==animation.id)beginInspection(animation);focusedAnimationId=animation.id;}
    focusMode=enabled;const id=focusedAnimationId??store.get().selectedAnimationId;if(id)sendCommand(frame(),{type:'SET_FOCUS_ANIMATION',id,enabled});
    tools.querySelector<HTMLButtonElement>('[data-focus-mode]')?.classList.toggle('active',enabled);
  };
  const setMagnifyMode=(enabled:boolean):void=>{
    if(enabled){const animation=selectedAnimation();if(!animation)return;pauseRecording();magnifiedAnimationId=animation.id;sendCommand(frame(),{type:'HIGHLIGHT_ANIMATION',id:animation.id});}
    const id=magnifiedAnimationId??store.get().selectedAnimationId;if(id)sendCommand(frame(),{type:'SET_MAGNIFY_ANIMATION',id,enabled});
    magnifyMode=enabled;if(!enabled)magnifiedAnimationId=undefined;
    tools.querySelector<HTMLButtonElement>('[data-magnify-mode]')?.classList.toggle('active',enabled);
  };
  const leaveSoloInspection=():void=>{
    if(focusMode)setFocusMode(false);sendCommand(frame(),{type:'CLEAR_SOLO_ANIMATION'});focusedAnimationId=undefined;window.dispatchEvent(new Event(ANIMATION_CLEAR_INSPECTION_EVENT));
  };
  const clearInspection=():void=>{
    if(magnifyMode)setMagnifyMode(false);leaveSoloInspection();isolatedElementId=undefined;isolation='all';applyIsolation();requestAnimationFrame(()=>centerPlayhead());
  };
  const setIsolation=(mode:Isolation):void=>{
    if(mode==='all'){clearInspection();return;}
    if(mode==='animation'){const animation=selectedAnimation();if(animation){beginInspection(animation);return;}}
    if(mode==='element'){
  ...
  const applyIsolation=():void=>{
    ...
    if(isolation==='animation')ids=new Set(state.selectedAnimationId?[state.selectedAnimationId]:[]);
      ...
      const elementId=isolatedElementId??selectedAnimation()?.elementId??state.selectedElementId;
    ...
    const label=tools.querySelector<HTMLElement>('[data-isolation-label]');if(label)label.textContent=isolation==='animation'?(state.selectedAnimationId?'1 motion':'Select motion'):isolation==='element'?(isolatedElementId?`${ids?.size??0} on element`:'Select element'):'All tracks';
  };
  const schedule=():void=>{if(raf)return;raf=requestAnimationFrame(()=>{raf=0;updateScale();applyIsolation();});};
  const click=(event:MouseEvent):void=>{const button=(event.target as Element|null)?.closest<HTMLButtonElement>('[data-timeline-zoom],[data-isolate],[data-focus-mode],[data-magnify-mode],[data-inspection-clear]');if(!button)return;const zoom=button.dataset.timelineZoom;if(zoom==='in')stepZoom(1);else if(zoom==='out')stepZoom(-1);else if(zoom==='fit')fit();else if(zoom==='frame')zoomFrame();else if(button.dataset.isolate)setIsolation(button.dataset.isolate as Isolation);else if(button.hasAttribute('data-focus-mode'))setFocusMode(!focusMode);else if(button.hasAttribute('data-magnify-mode'))setMagnifyMode(!magnifyMode);else clearInspection();};
  const key=(event:KeyboardEvent):void=>{const target=event.target as HTMLElement|null;if(target?.matches('input,textarea,select,[contenteditable="true"]'))return;if(event.key==='Escape'&&(focusMode||magnifyMode)){event.preventDefault();if(focusMode)setFocusMode(false);if(magnifyMode)setMagnifyMode(false);return;}const mod=event.ctrlKey||event.metaKey;if(mod&&(event.key==='+'||event.key==='=')){event.preventDefault();stepZoom(1);}else if(mod&&event.key==='-'){event.preventDefault();stepZoom(-1);}else if(mod&&event.key==='0'){event.preventDefault();fit();}else if(!mod&&event.key===']'){event.preventDefault();stepZoom(1);}else if(!mod&&event.key==='['){event.preventDefault();stepZoom(-1);}};
  ...
  const inspected=(event:Event):void=>{const detail=(event as CustomEvent<InspectionDetail>).detail;if(!detail)return;pauseRecording();focusedAnimationId=detail.id;isolatedElementId=detail.elementId;isolation='animation';applyIsolation();requestAnimationFrame(()=>viewport()?.scrollTo({left:0,behavior:'smooth'}));};
  ...
  return()=>{if(focusMode)setFocusMode(false);if(magnifyMode)setMagnifyMode(false);unsubscribe();observer.disconnect();if(raf)cancelAnimationFrame(raf);tools.removeEventListener('click',click);window.removeEventListener('keydown',key);window.removeEventListener(ANIMATION_INSPECT_EVENT,inspected);root.removeEventListener('wheel',wheel);tools.remove();};
}