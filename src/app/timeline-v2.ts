...
export function mountTimelineV2(root:HTMLElement):()=>void {
  ...
  const render=():void=>{
    ...
    viewport.style.setProperty('--timeline-label-width',`${preferredLabelWidth(groups,elements)}px`,'important');
  ...
  const selectAnimation=(animation:DetectedAnimation,inspect=false):void=>{
    pauseRecording();store.set({selectedAnimationId:animation.id,selectedElementId:animation.elementId.startsWith('static:')?store.get().selectedElementId:animation.elementId});sendCommand(frame(),{type:'HIGHLIGHT_ANIMATION',id:animation.id});if(!inspect)return;
