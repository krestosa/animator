...
import { TimelineEngine,TimelineVirtualizer,timelineRulerMarks,timelineTimeFromPointer,timelineTimeToPx,timelineTimingDuration,timelineTimingStart,timelineVisibleRange } from '../core/timeline';
...
export const TIMELINE_VIRTUALIZATION_EVENT='animator:timeline-virtualization-change';
...
type EventCluster={at:number;events:TimelineEvent[]};
type VirtualRow={key:string;height:number;html:string};
type RowHeights={group:number;instance:number;expanded:number;ruler:number};
...
export function mountTimelineV2(root:HTMLElement):()=>void {
  ...
  viewport.dataset.timelineIsolation='all';
  timeline.append(viewport);
  const expanded=new Set<string>(),expandedDetails=new Set<string>(),engine=new TimelineEngine(),virtualizer=new TimelineVirtualizer();
  let structuralSignature='',windowSignature='',eventSignature='',drag:DragState|undefined,raf=0,livePxPerMs=.1,viewOriginMs=0,inspectedAnimationId:string|undefined,lastProject='',rows:VirtualRow[]=[],rowHeights:RowHeights|undefined;
  ...
  const render=():void=>{
    ...
    if(projectKey!==lastProject){lastProject=projectKey;engine.reset();viewOriginMs=0;inspectedAnimationId=undefined;expanded.clear();expandedDetails.clear();structuralSignature='';windowSignature='';eventSignature='';rows=[];rowHeights=undefined;viewport.scrollTop=0;}
    ...
    if(signature!==structuralSignature){
      structuralSignature=signature;windowSignature='';eventSignature='';
      rowHeights=measureRowHeights(viewport);
      rows=flattenRows(groups,elements,expanded,expandedDetails,state.selectedAnimationId,pxPerMs,origin,rowHeights);
      virtualizer.setItems(rows);
    }
    renderRows(viewport,rows,virtualizer,rowHeights??measureRowHeights(viewport),groups.length,!!inspected,duration,pxPerMs);
    updateEvents(viewport,pxPerMs,origin,duration);
  };

  const renderRows=(view:HTMLElement,allRows:VirtualRow[],list:TimelineVirtualizer,heights:RowHeights,groupCount:number,inspected:boolean,duration:number,pxPerMs:number):void=>{
    const virtualizationEnabled=(view.dataset.timelineIsolation??'all')==='all'&&allRows.length>80;
    const bodyScrollTop=Math.max(0,view.scrollTop-heights.ruler),visibleHeight=Math.max(1,view.clientHeight-heights.ruler);
    const window=virtualizationEnabled?list.window(bodyScrollTop,visibleHeight,Math.max(260,visibleHeight)):{start:0,end:allRows.length,top:0,bottom:0,total:allRows.reduce((sum,row)=>sum+row.height,0)};
    const next=[structuralSignature,virtualizationEnabled?'v':'all',window.start,window.end,Math.round(window.top),Math.round(window.bottom)].join(':');
    if(next===windowSignature)return;
    windowSignature=next;eventSignature='';
    const left=view.scrollLeft,top=view.scrollTop,visible=allRows.slice(window.start,window.end);
    view.innerHTML=`<div class="v2Row v2RulerRow"><div class="v2Label v2Corner"><span>${inspected?'Isolated motion':'Animation lanes'}</span><small>${groupCount}</small></div><div class="v2Motion v2RulerMotion" data-v2-scrub-rail>${ruler(duration,pxPerMs)}</div></div>${virtualSpacer(window.top,'top')}${visible.map(row=>row.html).join('')}${virtualSpacer(window.bottom,'bottom')}<div class="v2Row v2EventRow"><div class="v2Label"><span class="v2LaneIcon event"></span><span class="v2GroupTitle">Events</span><small data-v2-event-count>0</small></div><div class="v2Motion" data-v2-event-motion></div></div>`;
    view.dataset.virtualized=String(virtualizationEnabled);view.dataset.virtualTotalRows=String(allRows.length);view.dataset.virtualRenderedRows=String(visible.length);
    if(view.scrollLeft!==left)view.scrollLeft=left;if(view.scrollTop!==top)view.scrollTop=top;
  };
  ...
  const syncInspection=(event:Event):void=>{const detail=(event as CustomEvent<InspectionDetail>).detail;if(!detail)return;inspectedAnimationId=detail.id;viewOriginMs=Math.max(0,detail.origin);structuralSignature='';schedule();};
  const selectAnimation=(animation:DetectedAnimation,inspect=false):void=>{
    ...
    sendCommand(frame(),{type:'HIGHLIGHT_ANIMATION',id:animation.id,reveal:inspect});
    if(!inspect)return;
    ...
    window.dispatchEvent(new CustomEvent<InspectionDetail>(ANIMATION_INSPECT_EVENT,{detail:{id:animation.id,elementId:animation.elementId,origin:viewOriginMs}}));schedule();
  };
  const click=(event:MouseEvent):void=>{
    const target=(event.target as Element|null)?.closest<HTMLElement>('[data-v2-detail-toggle],[data-v2-inspect],[data-v2-toggle],[data-v2-instance],[data-v2-group]');if(!target)return;
    if(target.dataset.v2DetailToggle){const key=target.dataset.v2DetailToggle;if(expandedDetails.has(key))expandedDetails.delete(key);else expandedDetails.add(key);structuralSignature='';schedule();event.stopPropagation();return;}
  ...
  const up=(event:PointerEvent):void=>{if(!drag||drag.pointerId!==event.pointerId)return;if(viewport.hasPointerCapture(event.pointerId))viewport.releasePointerCapture(event.pointerId);drag=undefined;viewport.classList.remove('isScrubbing');structuralSignature='';windowSignature='';eventSignature='';schedule();};
  ...
  function updateEvents(view:HTMLElement,pxPerMs:number,origin:number,duration:number):void{
    ...
    const next=`${pxPerMs}:${origin}:${duration}:${Math.round(range.start)}:${Math.round(range.end)}:${stats.version}:${windowSignature}`;if(next===eventSignature)return;eventSignature=next;
...
function flattenRows(groups:AnimationGroup[],elements:Map<string,RuntimeElement>,expanded:Set<string>,expandedDetails:Set<string>,selectedId:string|undefined,pxPerMs:number,originMs:number,heights:RowHeights):VirtualRow[]{
  const rows:VirtualRow[]=[];
  for(const group of groups){
    const detailOpen=expandedDetails.has(`g:${group.key}`);rows.push({key:`g:${group.key}`,height:detailOpen?heights.expanded:heights.group,html:groupRow(group,expanded,expandedDetails,selectedId,pxPerMs,originMs)});
    if(!expanded.has(group.key))continue;
    const runtime=group.instances.filter(instance=>!instance.elementId.startsWith('static:')),instances=runtime.length?runtime:group.instances;
    instances.forEach((instance,index)=>{const open=expandedDetails.has(`i:${instance.id}`);rows.push({key:`i:${instance.id}`,height:open?heights.expanded:heights.instance,html:instanceRow(instance,index,instances.length,elements,expandedDetails,selectedId,pxPerMs,originMs)});});
  }
  return rows;
}

function groupRow(group:AnimationGroup,expanded:Set<string>,expandedDetails:Set<string>,selectedId:string|undefined,pxPerMs:number,originMs:number):string {
  ...
  const start=Math.min(...instances.map(animationStart)),end=Math.max(...instances.map(item=>animationStart(item)+animationLength(item))),span=Math.max(1,end-start),detail=`${humanType(group.representative.type)} · ${instances.length} lane${instances.length===1?'':'s'} · ${formatTime(span)} span${group.representative.source?.selector?` · ${group.representative.source.selector}`:''}`,detailKey=`g:${group.key}`,detailOpen=expandedDetails.has(detailKey),long=needsDisclosure(title,detail),detailToggle=long?`<button class="v2DetailToggle${detailOpen?' open':''}" data-v2-detail-toggle="${attr(detailKey)}" aria-expanded="${detailOpen}" title="${detailOpen?'Collapse details':'Expand full label'}">${detailOpen?'⌄':'›'}</button>`:'';
  return `<div class="v2Row v2GroupRow${selected?' selected':''}${detailOpen?' v2DetailsExpanded':''}" data-animation-type="${attr(group.representative.type)}"><div class="v2Label"><button class="v2Disclosure" data-v2-toggle="${attr(group.key)}" title="${open?'Collapse lanes':'Expand into individual lanes'}">${open?'▾':'▸'}</button><span class="v2TypeDot" data-animation-type="${attr(group.representative.type)}"></span><button class="v2GroupButton" data-v2-group="${attr(representative.id)}" title="${attr(`${title} · ${detail}`)}"><span class="v2GroupTitle">${html(title)}</span><small>${html(detail)}</small></button><span class="v2LaneCount" title="${instances.length} animation lanes">${instances.length}</span>${detailToggle}<button class="v2Inspect" data-v2-inspect="${attr(representative.id)}" title="Inspect / isolate this animation">◎</button></div><div class="v2Motion">${aggregateClip(instances,representative,pxPerMs,originMs,start,end)}</div></div>`;
}

function instanceRow(instance:DetectedAnimation,index:number,total:number,elements:Map<string,RuntimeElement>,expandedDetails:Set<string>,selectedId:string|undefined,pxPerMs:number,originMs:number):string{
  const name=componentName(instance,elements),properties=instance.properties.map(item=>item.name).slice(0,3).join(', ')||'motion',source=componentDetail(instance,elements),detail=`${humanType(instance.type)} · ${formatTime(animationLength(instance))} · ${properties}${source?` · ${source}`:''}`,detailKey=`i:${instance.id}`,detailOpen=expandedDetails.has(detailKey),long=needsDisclosure(name,detail),detailToggle=long?`<button class="v2DetailToggle${detailOpen?' open':''}" data-v2-detail-toggle="${attr(detailKey)}" aria-expanded="${detailOpen}" title="${detailOpen?'Collapse details':'Expand full label'}">${detailOpen?'⌄':'›'}</button>`:'';
  return `<div class="v2Row v2InstanceRow${instance.id===selectedId?' selected':''}${detailOpen?' v2DetailsExpanded':''}" data-animation-type="${attr(instance.type)}" data-runtime-state="${attr(instance.runtimeState)}"><div class="v2Label v2InstanceLabel"><span class="v2Branch">${index===total-1?'└':'├'}</span><span class="v2TypeDot" data-animation-type="${attr(instance.type)}"></span><button data-v2-instance="${attr(instance.id)}" title="${attr(`${name} · ${detail}`)}"><span>${html(name)}</span><small>${html(detail)}</small></button>${detailToggle}<button class="v2Inspect" data-v2-inspect="${attr(instance.id)}" title="Inspect / isolate this animation">◎</button></div><div class="v2Motion">${clip(instance,pxPerMs,instance.id===selectedId,'v2InstanceClip',originMs)}</div></div>`;
}

function measureRowHeights(viewport:HTMLElement):RowHeights{
  const probe=(className:string):number=>{const node=document.createElement('div');node.className=`v2Row ${className}`;node.style.cssText='position:absolute!important;visibility:hidden!important;pointer-events:none!important;left:-100000px!important;top:0!important';node.innerHTML='<div class="v2Label"></div><div class="v2Motion"></div>';viewport.append(node);const height=Math.max(1,node.getBoundingClientRect().height);node.remove();return height;};
  return{group:probe('v2GroupRow'),instance:probe('v2InstanceRow'),expanded:probe('v2GroupRow v2DetailsExpanded'),ruler:probe('v2RulerRow')};
}
function virtualSpacer(height:number,position:'top'|'bottom'):string{return height>0?`<div class="v2VirtualSpacer" data-v2-spacer="${position}" style="height:${Math.max(0,height)}px"></div>`:'';}
function aggregateClip(instances:DetectedAnimation[],representative:DetectedAnimation,pxPerMs:number,originMs:number,startMs:number,endMs:number):string{const left=Math.max(0,(startMs-originMs)*pxPerMs),width=Math.max(6,(endMs-startMs)*pxPerMs),count=instances.length;return `<button class="v2Clip v2GroupClip${instances.some(item=>item.id===representative.id)?' representative':''}" data-v2-group="${attr(representative.id)}" data-animation-type="${attr(representative.type)}" style="left:${left}px;width:${width}px" title="${attr(`${count} lane${count===1?'':'s'} · ${formatTime(endMs-startMs)} total span`)}"><span class="v2ClipLabel">${count>1?`${count} lanes`:humanType(representative.type)}</span></button>`;}
function clip(animation:DetectedAnimation,pxPerMs:number,selected:boolean,className:string,originMs:number):string {const start=Math.max(0,(animationStart(animation)-originMs)*pxPerMs),duration=Math.max(1,animationLength(animation)*pxPerMs),label=animation.name??humanType(animation.type);return `<button class="v2Clip ${className}${selected?' selected':''}" data-v2-instance="${attr(animation.id)}" data-animation-type="${attr(animation.type)}" data-runtime-state="${attr(animation.runtimeState)}" style="left:${start}px;width:${Math.max(5,duration)}px" title="${attr(`${label} · ${formatTime(animationStart(animation))} start · ${formatTime(animationLength(animation))} duration · ${animation.easing??'linear'}`)}"><span class="v2ClipLabel">${html(label)}</span></button>`;}
function clusterEvents(events:TimelineEvent[],pxPerMs:number):EventCluster[]{const thresholdMs=8/Math.max(.0001,pxPerMs),sorted=[...events].sort((a,b)=>a.at-b.at),clusters:EventCluster[]=[];for(const event of sorted){const last=clusters.at(-1);if(last&&event.at-last.at<=thresholdMs){last.events.push(event);last.at=last.events.reduce((sum,item)=>sum+item.at,0)/last.events.length;}else clusters.push({at:event.at,events:[event]});}return clusters;}
...
function humanType(type:AnimationType):string{const labels:Record<AnimationType,string>={'css-animation':'CSS animation','css-transition':'Transition','web-animation':'WAAPI','javascript':'JavaScript','raf':'rAF','runtime-style':'Style write','gsap':'GSAP','framer-motion':'Framer Motion','scroll-timeline':'Scroll timeline','svg':'SVG','canvas':'Canvas','unknown':'Unknown'};return labels[type];}
