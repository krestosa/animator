...
const LABEL_MIN=228,LABEL_MAX=360;
...
export function mountTimelineV2(root:HTMLElement):()=>void {
  ...
  const render=():void=>{
    raf=0;const state=store.get(),groups=groupAnimations(state.animations,state.selectedAnimationId),elements=new Map(state.elements.map(element=>[element.id,element]));
    ...
    viewport.style.setProperty('--timeline-label-width',`${preferredLabelWidth(groups,elements)}px`);
    ...
    const signature=[state.zoom,state.selectedAnimationId??'',inspectedAnimationId??'',origin,groups.map(group=>groupSignature(group)).join('|'),[...expanded].sort().join(','),[...expandedDetails].sort().join(',')].join('::');
  ...
  const scrub=(event:PointerEvent,motion:HTMLElement):void=>{const pxPerMs=Number(viewport.dataset.pxPerMs??.1),duration=Number(viewport.dataset.duration??0),origin=Number(viewport.dataset.originMs??0),rect=motion.getBoundingClientRect();const local=Math.max(0,Math.min(duration,(event.clientX-rect.left)/pxPerMs)),time=origin+local;store.set({playhead:time});sendCommand(frame(),{type:'SCRUB_TIMELINE',time});};
  const canStartScrub=(event:PointerEvent,motion:HTMLElement):boolean=>{if(motion.matches('[data-v2-scrub-rail]'))return true;const rect=motion.getBoundingClientRect(),pxPerMs=Number(viewport.dataset.pxPerMs??.1),origin=Number(viewport.dataset.originMs??0),playheadX=Math.max(0,(store.get().playhead-origin)*pxPerMs);return Math.abs((event.clientX-rect.left)-playheadX)<=6;};
  ...
  function updateEvents(view:HTMLElement,events:TimelineEvent[],pxPerMs:number,origin:number):void{const recent=events.slice(-400),next=`${pxPerMs}:${origin}:${recent.map(item=>`${item.id}:${item.at}`).join(',')}`;if(next===eventSignature)return;eventSignature=next;const motion=view.querySelector<HTMLElement>('[data-v2-event-motion]'),count=view.querySelector<HTMLElement>('[data-v2-event-count]');if(!motion||!count)return;count.textContent=String(events.length);motion.innerHTML=recent.filter(item=>item.at>=origin).map(item=>`<i class="v2Event" title="${attr(item.label)}" style="left:${Math.max(0,(item.at-origin)*pxPerMs)}px"></i>`).join('');}
}
...
function groupRows(group:AnimationGroup,elements:Map<string,RuntimeElement>,expanded:Set<string>,expandedDetails:Set<string>,selectedId:string|undefined,pxPerMs:number,originMs:number):string {
  ...
  const detail=`${instances.length} component${instances.length===1?'':'s'} · ${group.representative.type}${group.representative.source?.selector?` · ${group.representative.source.selector}`:''}`,detailKey=`g:${group.key}`,detailOpen=expandedDetails.has(detailKey),long=needsDisclosure(title,detail);
  const detailToggle=long?`<button class="v2DetailToggle${detailOpen?' open':''}" data-v2-detail-toggle="${attr(detailKey)}" aria-expanded="${detailOpen}" title="${detailOpen?'Collapse details':'Expand full label'}">${detailOpen?'⌄':'›'}</button>`:'';
...
function preferredLabelWidth(groups:AnimationGroup[],elements:Map<string,RuntimeElement>):number{let chars=22,seen=0;for(const group of groups){chars=Math.max(chars,(group.representative.name??group.representative.type).length+12);for(const instance of group.instances){chars=Math.max(chars,componentName(instance,elements).length,componentDetail(instance,elements).length);if(++seen>=240)break;}if(seen>=240)break;}return Math.round(Math.min(LABEL_MAX,Math.max(LABEL_MIN,92+chars*4.4)));}
