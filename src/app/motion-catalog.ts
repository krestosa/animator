...
export function mountMotionCatalog(root:HTMLElement):()=>void{
  ...
  const select=(event:MouseEvent):void=>{const row=(event.target as Element|null)?.closest<HTMLElement>('.motionRow[data-animation-id]');if(row)pauseRecording();};
  const inspect=(event:MouseEvent):void=>{
    ...
    const animation=store.get().animations.find(item=>item.id===id);if(!animation)return;event.preventDefault();event.stopPropagation();pauseRecording();
