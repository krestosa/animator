...
export function mountControlSync():()=>void{
  ...
  const poll=async():Promise<void>=>{
    ...
    try{
      ...
      if(!active){if(controlledProjectId&&current?.browserSessionId===controlledProjectId)store.set({project:undefined,analysis:undefined,animations:[],events:[],elements:[],selectedElementId:undefined,selectedAnimationId:undefined,playhead:0});controlledProjectId='';return;}
