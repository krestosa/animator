...
describe('motion IR normalization',()=>{
  it('preserves legacy animation semantics',()=>{const track=detectedAnimationToMotionTrack(sample);expect(track.id).toBe(sample.id);expect(track.target).toEqual({elementId:'el1',selector:'.card'});expect(track.source.kind).toBe('waapi');expect(track.source.reference).toEqual(sample.source);expect(track.source.confidence).toBe(sample.confidence);expect(track.runtimeState).toBe(sample.runtimeState);expect(track.timing).toEqual({start:120,duration:300,delay:40,iterations:2,direction:'alternate',easing:'ease-in-out',fill:'both'});expect(track.properties).toEqual(sample.properties);expect(track.keyframes).toEqual([{offset:0,easing:'linear',values:{opacity:'0'}},{offset:1,values:{opacity:'1'}}]);});
