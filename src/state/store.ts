...
export const store={
  ...
  addAnimation(animation:DetectedAnimation):void{const normalized=correlateSource(animation,state.analysis);const index=state.animations.findIndex(item=>item.id===normalized.id);state={...state,selectedAnimationId:state.selectedAnimationId??normalized.id,animations:index>=0?state.animations.map(item=>item.id===normalized.id?correlateSource({...item,...normalized,startTime:item.startTime},state.analysis):item):[...state.animations,normalized]};emit();},
