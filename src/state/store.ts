...
type EditableSnapshot = Pick<DetectedAnimation,'duration'|'delay'|'easing'|'iterations'|'direction'|'fill'|'keyframes'>;
...
const applySnapshot=(animation:DetectedAnimation,value:EditableSnapshot):DetectedAnimation=>({...animation,...value,keyframes:value.keyframes?.map(frame=>({...frame}))});
...
export const store={
  ...
  updateAnimation(id:string,patch:Partial<DetectedAnimation>,record=true):void{
    ...
    const history=record?[...state.history,{id,before:snapshot(current),after:snapshot(updated)}]:state.history;
    state={...state,history,future:record?[]:state.future,animations:state.animations.map(animation=>animation.id===id?updated:animation)};emit();
  },
  addAnimation(animation:DetectedAnimation):void{
    const normalized=correlateSource(animation,state.analysis);const index=state.animations.findIndex(item=>item.id===normalized.id);
    state={...state,selectedAnimationId:state.selectedAnimationId??normalized.id,animations:index>=0?state.animations.map(item=>item.id===normalized.id?correlateSource({...item,...normalized,startTime:item.startTime},state.analysis):item):[...state.animations,normalized]};emit();
  },
