import type { MotionKeyframe,MotionTrack } from '../core/motion';
...
export const applyTrackSnapshot=(track:MotionTrack,value:EditableSnapshot):MotionTrack=>({...track,timing:{...track.timing,duration:value.duration,delay:value.delay,easing:value.easing,iterations:value.iterations,direction:value.direction,fill:value.fill},keyframes:value.keyframes.map(frame=>({...frame,values:{...frame.values}}))});
