...
export function installBlinkPreview(window:BrowserWindow):BlinkPreviewHandle{
  ...
  const disposeView=():void=>{
    const current=view;view=null;if(!current)return;
    try{window.contentView.removeChildView(current);}catch{}
    try{if(current.webContents.debugger.isAttached())current.webContents.debugger.detach();}catch{}
    if(!current.webContents.isDestroyed())current.webContents.close();
  };
  const applyViewport=(target:WebContentsView,value:Viewport):void=>{
    ...
    target.setBounds({x,y,width,height});target.webContents.setZoomFactor(zoom);target.setVisible(true);window.contentView.addChildView(target);
  };
  const ensureView=async():Promise<WebContentsView>=>{
    if(view)return view;
    const preload=fileURLToPath(new URL('./target-preload.js',import.meta.url));
    const next=new WebContentsView({webPreferences:{...(instrumentationEnabled?{preload}:{}),session:targetSession,nodeIntegration:false,contextIsolation:true,sandbox:true,spellcheck:false,backgroundThrottling:false}});
    ...
    if(lastViewport)applyViewport(next,lastViewport);
  ...
  const viewport=(value:Viewport):void=>{lastViewport=value;if(view)applyViewport(view,value);};
