...
async function runDiagnosticMode(window:BrowserWindow):Promise<void>{
  ...
  if(nativeSmoke){
    ...
    const target=`${server.origin}/api/health`,typedTarget=target.replace(/^http:\/\//,'');
    console.log(`Native Blink UI smoke test: opening protocol-less ${typedTarget}`);
    const clicked=await window.webContents.executeJavaScript(`(()=>{const details=document.querySelector('.webLoader'),input=document.querySelector('[data-web-url]'),button=document.querySelector('[data-web-open]');if(!(details instanceof HTMLDetailsElement)||!(input instanceof HTMLInputElement)||!(button instanceof HTMLButtonElement))return false;details.open=true;input.value=${JSON.stringify(typedTarget)};input.dispatchEvent(new Event('input',{bubbles:true}));button.click();return true;})()`,true) as boolean;
    if(!clicked)throw new Error('Native Blink UI smoke test: web loader controls unavailable');
    const projectReady=await pollRenderer<boolean>(window,`(()=>{const tab=document.querySelector('.workspaceTab.active');return Boolean(tab&&(tab.textContent||'').trim()!=='No project open');})()`,5000,false);if(!projectReady)throw new Error('Native Blink UI smoke test: button did not create a project');
    const popupClosed=await pollRenderer<boolean>(window,`document.querySelector('.webLoader')?.open===false`,3000,false);if(!popupClosed)throw new Error('Native Blink UI smoke test: web loader remained open after navigation');
